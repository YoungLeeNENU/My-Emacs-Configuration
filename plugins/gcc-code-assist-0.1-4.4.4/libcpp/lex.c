...
_cpp_lex_direct (cpp_reader *pfile)
{
 ...
 skipped_white:
  ...
  if (CPP_OPTION (pfile, phantom_token).type == CPP_CODE_COMPLETION)
    {
      unsigned int ins_line = CPP_OPTION (pfile, phantom_token).line;
      unsigned int ins_col = CPP_OPTION (pfile, phantom_token).column;

      if (ins_line)
	{
	  const struct line_map *map = linemap_lookup (pfile->line_table, result->src_loc);
	  unsigned int cur_line = SOURCE_LINE (map, result->src_loc);
	  unsigned int cur_col = SOURCE_COLUMN (map, result->src_loc);

	  if (((ins_line == cur_line && ins_col <= cur_col)
	       || ins_line < cur_line)
	      && !strcmp (CPP_OPTION (pfile, phantom_token).file, map->to_file))
	    {
	      /* Revert advance.  */
	      buffer->cur--;
	      
	      /* Deactivate phantom token in next time.  */
	      CPP_OPTION (pfile, phantom_token).line = 0;
            
	      result->type = CPP_OPTION (pfile, phantom_token).type;
	      return result;
	    }
	}
    }
