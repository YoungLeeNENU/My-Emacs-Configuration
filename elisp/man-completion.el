...
;; filenames, and a new default page name at point which recognises more
;; things, and optionally can transformation some Perl class names.  See
;; `man-completion-read' docstring for details.
;;
...
;; catdoc(1) then you must type "cat(" to differentiate, which is a bit
;; annoying.
;;
...
;; command line.  But the strategy in man-completion.el here is to ask the
;; man program what pages it knows rather than digging in the filesystem.
;; This helps include aliases man knows from its lexgrog parse and which
;; don't have symlinks (or ".so" pages), and it may also help on strange
...
;; Explicit dependency on advice.el so it's not unloaded before us.  Need
;; `ad-find-advice' macro when running uncompiled and that macro isn't
;; autoloaded.
(require 'advice)
...
Return an expanded string, or nil if nothing found."
  ...
  (eval-and-compile (require 'cl)) ;; for `remove*', `remove-duplicates'
  (let* ((case-fold-search nil)
    ...
    (let ((str
           (if (man-completion-looking-at-brack) ;; special case for "["
             ...
             (save-restriction
               ...
               ;; Narrow to start of previous line to limit how far
               ;; thing-at-point-looking-at searches.  (The previous line so
               ;; that a hypenated foo-\nbar becomes foobar when point is on
               ;; the bar part.)
               ;;
               (save-excursion
                 (forward-line -1)
               ...
               ;; page names is unusual and wanting to visit it from pod
               ;; markup even more unusual.  New enough pod accepts actual
               ;; non-ascii chars too anyway, which need no special
               ;; handling.
               ;;
               (save-excursion
...
Return true if the files have changed.  Update the last noted
...
;; LocalWords: catdoc usr lexgrog symlinks manpath conf db modtimes groff latin initializes initialized parenthesized DESC manpage minibuffer pagename Xyzzy multipart Yy Quux Filename filenames docstring el filesystem lookup perl eg ascii unicode whitespace nroff config MFoo dir
