...
;; Designed for Emacs 21 up.  Works in XEmacs 21 though there's no "nobreak"
;; feature there.  Partly works in Emacs 20 (`perl-pod-gt-single' no good,
...
;; the hooks if you know how to use `update-file-autoloads' and friends.
...
an end marker."
  ...
  (let ((case-fold-search nil))
    (save-excursion
      ...
      (while (and (re-search-forward "\\(E<lt>\\)+\\|\\(E<gt>\\)+\\|\\(>\\)"
                                     nil t)
                  (cond ((match-beginning 3) ;; final ">"
                         (replace-match " >>" t t)
                         nil) ;; stop loop
                        (t
                         ;; only an isolated E<gt> or E<lt>, not multiple
                         ;; ones as don't want to look like closing ">>"
                         (if (= 5 (- (match-end 0) (match-beginning 0)))
...
;; LocalWords: el gt versa formatters perl whitespace nroff troff nobreak
